...
const handleGenerateResponse = (response) => {
    if (response.code == 1 && response.authenticationInformation) {
        ...
        injectDataCollectionFrame(deviceDataCollectionUrl, accessToken, _ => {
            alert('Coleta de dados 3DS finalizada com sucesso!');
            
            document.getElementById('creditCard-form-container').style.display = 'none';
            document.getElementById('3ds-form-container').style.display = 'block';
        });
    }
}

const handle3DSSubmit = (event) => {
    event.preventDefault();

    const url = document.getElementById('url').value;
    const accessToken = document.getElementById('accessToken').value;
    const paRequest = document.getElementById('paRequest').value;
